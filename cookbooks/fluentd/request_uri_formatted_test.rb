...
class RequestUriFormattedTest < Test::Unit::TestCase
  def format_request_uri(request_uri)
    request_uri.gsub(%r{(?<=/)[0-9]+(?=(/|$|\.))}, ":id").gsub(%r{(?<=/)[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}(?=(/|$|\.))}, ":uuid").gsub(%r{(?<=/)[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}[^/]*$}, ":uuid_with_random")
  end
